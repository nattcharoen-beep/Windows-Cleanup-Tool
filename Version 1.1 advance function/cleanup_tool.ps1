...
function Clean-SystemJunk {
    ...
    try {
        if ($Global:PreviewMode -eq $false) {
            ...
            Write-Success "[Preview] Will empty the trash can."
        }
    } catch {
...
function Repair-Registry {
            ...
            foreach ($key in $uninstallKeys) {
                ...
                if (-not [string]::IsNullOrWhiteSpace($installLocation)) {
                    if (-not (Test-Path -Path $installLocation -ErrorAction SilentlyContinue)) {
                        ...
                        if ($Global:PreviewMode -eq $false) {
                            # Do not touch Windows Updates or system components
    ...
    if ($invalidEntriesCount -gt 0) {
        Write-Success "Detects and cleans a number of residual registry entries. $invalidEntriesCount list"
...
function Tune-Performance {
    ...
    try {
        ...
        $diskInfo = Get-PhysicalDisk -ErrorAction SilentlyContinue | Where-Object { $_.DeviceID -eq 0 -or $_.OperationalStatus -eq "OK" } | Select-Object -First 1 -Property MediaType
...
- Total working time:   $($timeTaken.Minutes) minute $($timeTaken.Seconds) seconds
...
function Optimize-Background {
    ...
    # Reduce Telemetry
    Update-RegSetting -Desc "Reduces Windows Telemetry data collection to the basic level." `
                      -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\DataCollection" `
                      -Name "AllowTelemetry" -NewValue 1
...
function Analyze-DiskSpace {
    ...
    try {
        ...
        foreach ($drv in $allDrives) {
            ...
            $allFiles += Get-ChildItem -Path "$drv\" -File -Recurse -ErrorAction SilentlyContinue | Sort-Object Length -Descending | Select-Object -First 50
        }
    ...
    try {
        $files = Get-ChildItem -Path "C:\Users\" -File -Recurse -ErrorAction SilentlyContinue | Where-Object { $_.Length -gt 1MB }
...
function Optimize-Network {
    ...
    # Reset TCP/IP stack
    Write-Host "`nResetting the network (TCP/IP Stack & Winsock)..." -ForegroundColor Cyan
    if (-not $Global:PreviewMode) {
        try {
    ...
    } else { Write-Host "  [PREVIEW] Run command netsh int ip reset / netsh winsock reset" -ForegroundColor DarkGray }
                ...
                try {
                    ...
                    if ($uninstallString) {
                        if ($uninstallString -match "msiexec") {
                            ...
                            Start-Process $uninstallString -ArgumentList "/S", "/quiet" -Wait -NoNewWindow -ErrorAction SilentlyContinue
                        }
...
function Repair-SystemDeep {
    ...
    if (-not $Global:PreviewMode) {
        ...
        Write-Host "\nRunning Deployment Image Servicing and Management (DISM)..." -ForegroundColor Cyan
...
function Show-MainMenu {
    while ($true) {
        ...
        $choice = Read-Host "`nPlease select a menu (1-14, A, P, L, Q)"
...
function Clean-SystemJunk {
    ...
    try {
        if ($Global:PreviewMode -eq $false) {
            ...
            Write-Success "[พรีวิว] จะทำการล้างถังขยะ"
        }
    } catch {
...
function Repair-Registry {
    ...
    if ($invalidEntriesCount -gt 0) {
        Write-Success "ตรวจพบและทำความสะอาดรายการรีจิสทรีที่ตกค้างจำนวน $invalidEntriesCount รายการ"
        $Global:TotalFixed += $invalidEntriesCount
    } else {
...
- เวลาที่ใช้ทำงานทั้งหมด:   $($timeTaken.Minutes) นาที $($timeTaken.Seconds) วินาที
...
function Optimize-Background {
    ...
    # Reduce Telemetry
    Update-RegSetting -Desc "ลดการเก็บข้อมูล (Telemetry) ของ Windows เป็นระดับพื้นฐาน" `
                      -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\DataCollection" `
                      -Name "AllowTelemetry" -NewValue 1
    ...
    if (-not $printers -or $printers.Count -eq 0) {
        $servicesToCheck += "Spooler"
        Write-Host "  ไม่พบปริ้นเตอร์จริงในระบบ เพิ่มบริการ Spooler ในรายการปิด" -ForegroundColor DarkGray
    }
...
function Optimize-Network {
    ...
    # Reset TCP/IP stack
    Write-Host "`nกำลังรีเซ็ตเครือข่าย (TCP/IP Stack & Winsock)..." -ForegroundColor Cyan
    if (-not $Global:PreviewMode) {
        try {
            netsh int ip reset | Out-Null
            netsh winsock reset | Out-Null
    ...
    } else { Write-Host "  [PREVIEW] รันคำสั่ง netsh int ip reset / netsh winsock reset" -ForegroundColor DarkGray }
...
function Setup-AutoMaintenance {
    ...
    Write-Host "6. สร้างไอคอนเข้าโปรแกรมนี้บนหน้าจอ" -ForegroundColor White
}
...
function Repair-SystemDeep {
    ...
    if (-not $Global:PreviewMode) {
        ...
        Write-Host "\nกำลังรัน DISM (RestoreHealth)..." -ForegroundColor Cyan
        DISM /Online /Cleanup-Image /RestoreHealth
...
function Show-MainMenu {
    while ($true) {
        ...
        $choice = Read-Host "`nโปรดเลือกเมนู (1-14, A, P, Q)"
...
while ($true) {
    Clear-Host
    Write-Host "==================================================" -ForegroundColor Cyan
    Write-Host "            Select Language / เลือกราษา            " -ForegroundColor Cyan
    Write-Host "==================================================" -ForegroundColor Cyan
    Write-Host "1. English" -ForegroundColor White
    Write-Host "2. ภาษาไทษ" -ForegroundColor White
